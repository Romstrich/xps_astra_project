'''
    ...
    ---------------------------------------------------------------------
    Версия 0.4.2 для xps_astra 0.4.x, 0.5.x

    0.4.2
        - *вынесение части функционала в модуль випнет
        - передача параметра прав, исключение лишних проверок
        - исключение передачи пароля в буфер обмена
        - *проверка и включение автозапуска ViPNet-client

        Модуль работает без ключей - режим поиска файла xps на рабочем
    столе, вывод содержимого первой страницы, выявление пароля.
        Ключи:
